...
引擎从 1900 行重构为 1089 行（-43%），改为**工作流定义驱动的纯图遍历器**：

- **边定义流向**：forward / reject / escalate / timeout / fallback 五种边的语义决定节点间如何流转
- **`_resolve_next_edge`**：统一的边解析函数，条件/路由/校验节点的所有流向逻辑集中处理
- **并行打回**：当 router 有多个 reject 边时，自动并行执行所有回退目标（`asyncio.gather`）
- **循环保护**：`visit_count` 上限防止无限回退

### HITL 审核关卡（3 关 · 5 打回路径）

```
需求分析 → 👀 需求评审(HITL) → 🔀 条件 → 打回需求/继续
方案设计 → 👀 方案评审(HITL) → 🔀 条件 → 打回方案/继续  
后端+前端并行 → 👀 代码评审(HITL) → 🔀 路由 → 
  ├─ ✅ 通过 → 集成测试
  ├─ ❌ 打回后端 → reject → 💻 后端开发
  ├─ ❌ 打回前端 → reject → 🌐 前端开发
  └─ 🔄 全部打回 → 并行回退 💻+🌐
...
- **暂停状态 DB 持久化**：HITL 暂停时自动创建 `WorkflowInstance` 记录
- **WS 重连推送**：重连后立即发送 HITL 卡片
- **前端轮询**：WS 重连后每 3s 轮询历史，Agent 消息到达后自动停止
